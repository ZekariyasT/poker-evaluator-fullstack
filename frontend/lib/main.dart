...
class _HandEvaluatorScreenState extends State<HandEvaluatorScreen> {
  ...
  Set<String> get _usedCards => {...hole, ...community};

  void _evaluate() async {
    if (hole.length != 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select 2 hole cards.')),
      );
      return;
    }
    if (community.length != 5) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select 5 community cards.')),
      );
      return;
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _CardList(
            cards: community,
            onClear: () => setState(() => community.clear()),
          ),
          const SizedBox(height: 24),
          _CardPicker(
          ...
          Center(
            child: ElevatedButton.icon(
              onPressed: loading ? null : _evaluate,
              icon: loading
                  ? const SizedBox(
                      width: 16,
                      height: 16,
...
class _CompareHandsScreenState extends State<CompareHandsScreen> {
  ...
  Set<String> get _usedCards => {...p1, ...p2, ...community};

  void _compare() async {
    if (p1.length != 2 || p2.length != 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Both players must have 2 hole cards.')),
      );
      return;
    }
    if (community.length != 5) {
...
class _MonteCarloScreenState extends State<MonteCarloScreen> {
  ...
  void _run() async {
    ...
    final n = community.length;
    if (n != 0 && n != 3 && n != 4 && n != 5) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Community cards must be exactly 0 (Pre-flop), 3 (Flop), 4 (Turn), or 5 (River).',
          ),
        ),
      );
      return;
    ...
    try {
      final res = await widget.client.getProbabilities(
        hole,
        community,
        players,
        simulations,
      ...
      setState(() {
        result =
            'Win Probability: ${(res.winProbability * 100).toStringAsFixed(2)}%';
        loading = false;
      });
    } catch (e) {
...
class _CardPicker extends StatelessWidget {
  ...
  const _CardPicker({required this.onPick, this.usedCards = const {}});
            ...
            children: [
              Padding(
                ...
                child: Wrap(
                  ...
                  spacing: 4,
                  runSpacing: 4,
                  children: Rank.values.map((r) {
                    final code = '${s.code}${r.code}';
                    final isUsed = usedCards.contains(code);
                    return GestureDetector(
                      onTap: isUsed ? null : () => onPick(code),
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 150),
                        width: 38,
                        height: 50,
                        decoration: BoxDecoration(
                          color: isUsed ? Colors.grey.shade800 : Colors.white,
                          borderRadius: BorderRadius.circular(4),
                          border: isUsed
                              ? Border.all(
                        ...
                        child: Center(
                          child: Text(
                            '${r.code}${s.symbol}',
                            style: TextStyle(
                              color: isUsed ? Colors.grey.shade600 : s.color,
                              fontWeight: FontWeight.bold,
                              fontSize: 14,
                              decoration: isUsed
